	...
	void Update () {
        ...
        if (m_CurrCount < m_MaxCount)
        {
			if (Time.time > m_PrevCreatTime + UnityEngine.Random.Range(1.5f, 3.5f))
			{
				...
				objClone.transform.position = transform.TransformPoint(new Vector3(UnityEngine.Random.Range(0.5f, 0.5f), 0, UnityEngine.Random.Range(-.05f, 0.5f)));
				...
				roleInfo.RoleID = 1;
				roleInfo.CurrHP = roleInfo.MaxHP = 10000;
